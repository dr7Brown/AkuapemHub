...
- Settings menu MVP (account: profile photo edit + close/deactivate account; role: switch between customer and worker via `become_worker.php`/`switch_to_customer.php`, with quick reactivation of an existing worker profile when switching back; location: town + GPS grouped under a clear section; notifications: email on/off preference wired into `send_email_notification()` so opted-out users no longer receive emails)
