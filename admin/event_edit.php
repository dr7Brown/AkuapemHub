        ...
        if ($id) {
            $pdo->prepare(
                "UPDATE events SET title=?,slug=?,featured_image=?,description=?,venue=?,gps_address=?,
                 ...
                 ticket_type=?,ticket_price=?,registration_link=?,status=?,featured=?,updated_at=NOW()
                 WHERE id=?"
            )->execute([$title,$slug,$imgPath,$desc,$venue,$gps,$startDate,$endDate,$startTime,$endTime,
                        $organizer,$ticketType,$ticketPrice,$regLink,$status,$featured,$id]);
            ...
            $pdo->prepare(
                "INSERT INTO events (title,slug,featured_image,description,venue,gps_address,start_date,end_date,
                 start_time,end_time,organizer_name,ticket_type,ticket_price,registration_link,status,featured)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
            )->execute([$title,$slug,$imgPath,$desc,$venue,$gps,$startDate,$endDate,$startTime,$endTime,
...
<body>
    ...
    <div class="ee-shell">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="ee-field"><label>Venue</label><input type="text" name="venue" class="form-control" value="<?php echo $v('venue'); ?>" placeholder="Hall name, church, stadium…"></div>
            <div class="ee-field"><label>GPS Address</label><input type="text" name="gps_address" class="form-control" value="<?php echo $v('gps_address'); ?>" placeholder="e.g. GA-123-4567"></div>
